...
export default function MLBApp({ sport, setSport }) {
  ...
  const TeamComparisonView = () => {
    if (!teamStats || !compareTeam || !compareTeamStats) return null;

    const baseAbbr = selectedTeamInfo.abbr;
    const cmpAbbr = compareTeam.abbr;
    const baseColor = teamColors[baseAbbr] || '#3B82F6';
    const cmpColor = teamColors[cmpAbbr] || '#EF4444';

    const getStat = (statsArr, catName, statName) => {
    ...
    const compareSections = [
      {
        title: 'Batting',
        stats: [
          { label: 'AVG', catName: 'batting', statName: 'avg', format: 'avg' },
          { label: 'OBP', catName: 'batting', statName: 'onBasePct', format: 'avg' },
          { label: 'SLG', catName: 'batting', statName: 'slugAvg', format: 'avg' },
          { label: 'OPS', catName: 'batting', statName: 'OPS', format: 'avg' },
      ...
      },
      {
        title: 'Pitching',
        stats: [
          { label: 'ERA', catName: 'pitching', statName: 'ERA', inverse: true },
          { label: 'WHIP', catName: 'pitching', statName: 'WHIP', inverse: true },
          { label: 'K/9', catName: 'pitching', statName: 'strikeoutsPerNineInnings' },
          { label: 'BB/9', catName: 'pitching', statName: 'walksPerNineInnings', inverse: true },
      ...
      },
      {
        title: 'Fielding',
        stats: [
          { label: 'FPCT', catName: 'fielding', statName: 'fieldingPct', format: 'avg' },
    ...
    return (
      <div style={{ animation: 'slideInRight 0.3s ease-out' }}>
        {/* Comparison header */}
        <div className="flex items-center justify-between mb-4 bg-zinc-900 rounded-2xl p-4">
          <div className="flex flex-col items-center flex-1 cursor-pointer" onClick={() => { setCompareTeam(null); setCompareTeamStats(null); setIsCompareMode(false); }}>
            <img src={selectedTeamInfo.logo} alt={baseAbbr} className="w-14 h-14 mb-1" />
            <span className="font-bold text-sm" style={{ color: baseColor }}>{baseAbbr}</span>
            <span className="text-xs text-gray-400">{teamStats.record?.wins}-{teamStats.record?.losses}</span>
          </div>
          <div className="flex flex-col items-center px-2">
            <span className="text-gray-500 text-xs font-bold uppercase tracking-widest">VS</span>
            <button
              onClick={() => { setCompareTeam(null); setCompareTeamStats(null); setIsCompareMode(false); }}
              className="mt-2 text-xs text-gray-500 hover:text-white bg-zinc-800 px-2 py-1 rounded-lg"
            >
              Change
            </button>
          </div>
          <div className="flex flex-col items-center flex-1 cursor-pointer" onClick={() => handleTeamClick(cmpAbbr, compareTeam.logo)}>
            <img src={compareTeam.logo} alt={cmpAbbr} className="w-14 h-14 mb-1" />
            <span className="font-bold text-sm" style={{ color: cmpColor }}>{cmpAbbr}</span>
        ...
        {compareSections.map(section => (
          <div key={section.title} className="bg-zinc-900 rounded-2xl p-4 mb-4">
            <h4 className="text-xs font-semibold text-gray-400 uppercase tracking-widest mb-4" style={{ fontFamily: 'Rajdhani, sans-serif' }}>
              {section.title}
            </h4>
            <div className="space-y-4">
              {section.stats.map(({ label, catName, statName, inverse, format }) => {
                const bv = getStat(teamStats.stats, catName, statName);
                const cv = getStat(compareTeamStats.stats, catName, statName);
                const total = bv + cv;
                ...
                return (
                  <div key={label}>
                    <div className="flex justify-between items-center mb-1.5">
                      <span className={`text-base font-bold w-16 ${baseBetter ? 'text-white' : 'text-gray-500'}`}>{fmt(bv)}</span>
                      <span className="text-gray-400 text-xs font-semibold flex-1 text-center">{label}</span>
                      <span className={`text-base font-bold w-16 text-right ${cmpBetter ? 'text-white' : 'text-gray-500'}`}>{fmt(cv)}</span>
                    </div>
                    <div className="flex h-2 rounded-full overflow-hidden bg-zinc-800">
                      <div style={{ width: `${bp}%`, backgroundColor: baseColor, opacity: baseBetter ? 1 : 0.5 }} />
                      <div style={{ width: `${cp}%`, backgroundColor: cmpColor, opacity: cmpBetter ? 1 : 0.5 }} />
                    </div>
                  </div>
                );
              })}
            ...
            <div className="max-w-2xl mx-auto">
              <div className="flex items-center mb-6 sticky top-0 z-50 py-3 px-1 backdrop-blur-md border-b border-white/5" style={{ background: 'linear-gradient(to bottom, rgba(0,0,0,1) 0%, rgba(0,0,0,0.7) 100%)' }}>
                ...
                <h2 className="text-2xl font-bold" style={{ fontFamily: 'Rajdhani, sans-serif' }}>
                  {compareTeam && compareTeamStats ? 'Team Comparison' : 'Team Stats'}
                </h2>
              </div>
              ...
              {loadingTeamStats ? (
                ...
                <div>
                  ...
                  >
                    ...
                    <div className="flex items-start gap-4 relative z-10 mb-4">
                      <div className="flex flex-col items-center gap-2">
                        ...
                        <button
                          onClick={() => { setIsCompareMode(true); setCompareTeam(null); setCompareTeamStats(null); }}
                  ...
                  {/* ── REGULAR STATS (hidden when comparing) ── */}
                  {!compareTeam && (
                    <>
                      {/* Batting */}
