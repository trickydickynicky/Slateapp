...
const fetchGameTeamRecords = async (awayAbbr, homeAbbr) => {
  ...
  useEffect(() => {
    if (!showStandings || !standingsRef.current) return;
  ...
  }, [showStandings, standingsRef.current]);

  useEffect(() => {
    if (!selectedGame || !gameDetailRef.current) return;
    const el = gameDetailRef.current;
...
<div className="flex mb-1" style={{ animation: 'fadeUp 0.45s cubic-bezier(0.22, 1, 0.36, 1) 0.15s both' }}>
  <div className="flex bg-zinc-900 rounded-full p-0.5 gap-0.5">
    {['NBA', 'MLB'].map(s => (
      <button key={s} onClick={() => { setSport(s); trigger('selection'); }}
...
{selectedGame && (
  <div 
    ref={(el) => { gameDetailScrollRef.current = el; gameDetailRef.current = el; }}
...
<span className="text-2xl font-light" style={{ transform: showAllUpcoming ? 'rotate(90deg)' : 'rotate(-90deg)' }}>      ‹
  ...
  {selectedNBAPlayer && (
  <div ref={nbaPlayerRef} className="fixed inset-0 bg-black bg-opacity-100 z-[140] overflow-y-auto animate-[slideInRight_0.3s_ease-out]" style={{
